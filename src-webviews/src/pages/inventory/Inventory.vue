    ...
    <div class="split-three-way" oncontextmenu="return false;">
                ...
                <template v-slot:content>
                    <div class="split-interface">
                        ...
                        <div class="split split-full center pb-4 pt-4" v-if="split.item.quantity - 1 >= 2">
                            ...
                            <RangeInput :minIndex="1" :maxIndex="split.item.quantity - 1" :indexValue="splitAmount"
                                :increment="1" @input="(e) => setIncrementAmount(e, null)"
        ...
        <div class="equipment">
            <div class="equipment-grid">
                <div v-for="(item, index) in equipment" :id="`e-${index}`"
                    :class="getItemClass(item, index, equipmentSize)" :key="index" v-on="
                        !item
                            ? {}
                            : {
        ...
        <div class="character">
            ...
            <div class="toolbar-grid">
                <div v-for="(item, index) in toolbar" :key="index" :id="`t-${index}`"
                    :class="!item ? { 'is-null-item': true } : { item: true }" v-on="
                        !item
                            ? {}
                            : {
                                mousedown: selectItem,
        ...
        <div class="inventory">
            ...
            <div class="inventory-grid">
                <div v-for="(item, index) in inventory" :id="`i-${index}`" :key="index"
                    :class="getItemClass(item, index, inventorySize)" v-on="
                        !item
                            ? {}
                            : {
                                mousedown: selectItem,
...
export default defineComponent({
    ...
    methods: {
        ...
        updateInventory(items: Array<{ slot: number }>, size = 20) {
        ...
        updateToolbar(items: Array<{ slot: number }>, size = 3) {
            const sizeToUse = items.length > size ? items.length : size;
    ...
    mounted() {
        if (!('alt' in window)) {
            ...
            this.toolbar = new Array(3).fill(null);
...
.key-group {
    ...
    margin-top: 3px;
    padding-top: 3px;
    border-top: 2px solid rgba(248, 248, 248, 0);
    overflow-wrap: break-word;
...
.inventory-grid >div{
      background-color: rgba(57, 57, 96, 0.8);
border-radius: 2px;
      padding: 10px;
      border-radius: 3px;
}
